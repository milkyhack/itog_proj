...
<body>
     <div class="header">
      <div class="container">
        ...
        <div class="form">
          <input type="text" v-bind:value="valueInput"  v-on:input="handleInput" v-on:keypress.enter="addTask">
          <button class="btn" v-on:click="addTask">Добавить</button>
        </div>
      </div>
    </div>
    <div class="container">
      ...
      <ul class="mask-list">
        <li v-for="(mask, index) in needDoList" :key="mask.id">
          ...
          <button class="btn-remove" v-on:click="removeMask(index, 'need')">Убрать</button>
        </li>
      </ul>
      <h2>
        <span>Выполнил</span>
      ...
      <ul class="mask-list complete-list">
        <li v-for="(mask, index) in completeList" :key="mask.id" >
          ...
          <button class="btn-remove" v-on:click="removeMask(index, 'complete')">Убрать</button>
        </li>
      </ul>
    </div>
 </body>
...
export default {
     data() { 
             return {valueInput: '', needDoList: [],completeList: []
    };
  },
  methods: {
    handleInput (event) {
...
.header {
  background-color: rgb(13, 100, 57);
  padding: 10px 0;
  margin-bottom: 40px;
}
